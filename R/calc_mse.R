#' Calculate mean squared error (MSE) for a vector or a matrix of observed and predicted values
#'
...
#'
#' @return MSE or $R^2$ (single value if vectors are passed or a vector of length \code{nrow(obs)} if matrices/df are passed, one value for each obs-pred column pair
